...
const rootStyling =
  'bg-white rounded-full w-full cursor-pointer hover:bg-accent-secondary hover:ring-transparent hover:-translate-y-1 duration-200 group dark:bg-neutral-900 transition-color'
...
const outputRootStyle = computed(() => {
  ...
  if (props.altColor) {
    newStyle = newStyle
      .replace('bg-white', 'bg-transparent')
      .replace('hover:bg-accent-secondary', 'hover:bg-white')
  }
...
const txtStyle =
  'text-neutral-800 group-hover:text-white dark:text-white dark:group-hover:text-neutral-900'
